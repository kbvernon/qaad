...
2. Aggregate data
    - Loss of information, particular within-group differences
...
gg_responses <- ggplot(responses) +
  geom_ribbon(
    aes(elevation, ymin = ymin, ymax = ymax, group = survey),
    fill = "gray80",
    alpha = 0.5
  ) +
  geom_line(
    aes(elevation, residential, color = survey),
    size = 1.3
  ) +
  geom_text(
    data = facet_labels,
...
Can calculate mean and variance of coefficients across all groups $j$ in a post-hoc fashion, using a model's coefficient estimates $\hat\beta$

$$\bar\beta = \frac{1}{j} \sum_{i=1}^{j} \hat\beta_{j}$$

$$Var(\bar\beta)=\frac{1}{j} \sum_{i=1}^{j} (\hat\beta_{j}-\bar\beta)^2$$

but, it's better to calculate these parameters directly from the data as part of the model fitting process.
...
- $W$ is some property of the group
...
gg_mixedeffects <- ggplot(responses, aes(x = x, y = predicted)) +
  geom_ribbon(
