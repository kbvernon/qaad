...
Saving a ggplot is simple enough. You use the `ggsave()` function, specify the plot you want to save, and tell it where you want to save it. Here is an example:
...
There are two things to note here. First, you can specify the dimensions of the resulting figure with width and height (the unit for these is inches). Second, check out the weird "../" in the `filename` argument. What is that about? Well, right now, the working directory for your R Markdown document is the `R` folder within your `qaad` project folder, as shown in the schematic below.
...
|-+ qaad
  ...
  |-+ figures
  |-+ R
    |-+ lab01-introduction.Rmd
    |-+ lab02-statistical_graphics.Rmd
```

However, you want to save the plot into your `figures` folder, which is at the same level as `R`. So, what the file path "../figures/penguins.png" says is "go up one level from your current directory in R to the qaad directory, then go back down into the figures folder, and put the penguins.png file there." Later on, I'll introduce you to the `here` package, which makes writing file paths in your project folder a lot easier. For now, though, we'll just use this old-school technique for generating a file path.

### &#x1F3CB; Exercises {.exercises}

1. Run the code above to save the `penguins_plot` to your figures folder.
...
3. **Graphics.** And now to visualize them.
    ...
    - Finally, assign your graphic to object, maybe `dartpoints_plot` and save that to the figures folder in your project directory with `ggsave()`. (Hint: make sure to use "../" in the filename.)
